...
data_dir = "/scr-ssd/mengyuan/gen_data/data_with_knots/"

eval_snapshot = "./b-spline_data_pred_knots/model-29"
...
def experiment(test_idx, option='diagnose'):
    ...
    pred = model.predict_single(sess, im)
    pred_knots = [pred[:,0], pred[:,0], pred[:,1],
                  pred[:,2], pred[:,3], pred[:,3]]
    ...
    if option == "animation":
        ...
        output_dir = 'rollout_knot_experiments/rollout_knot_%d'%(test_idx)
        os.mkdir(output_dir)
