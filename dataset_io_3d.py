...
def data_writer(img, position, intersections):
    height = img.shape[0]
    width = img.shape[1]
    ...
    record = tf.train.Example(features=tf.train.Features(feature={
        ...
        'intersect': _intList_feature(intersections.flatten().tolist())}))
    return record
...
def data_parser(record):
    features = tf.parse_single_example(
      ...
      features={
        ...
        'intersect': tf.FixedLenFeature([127], tf.int64)
        })
    ...
    intersect = features['intersect']+1
    intersect = tf.one_hot(intersect, depth=3)
    return image, position, intersect
