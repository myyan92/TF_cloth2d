...
#        self.reg_loss_e = self.reg_loss_e * 10
    ...
    def setup_optimizer(self):
        self.setup_image_loss()
        self.reg_loss_l2 = tf.losses.get_regularization_loss()

        prob_grad = tf.gradients(self.image_loss, self.pix_prob)[0]
        ...
        image_grads = tf.gradients(self.pix_prob, vars, grad_ys=prob_grad)
        gradvars_new = []
...
if __name__ == "__main__":
    sess = tf.Session()
    model = Model_IM_EM_v2(vgg16_npy_path='./vgg16_weights.npz', fc_sizes=[1024, 256],
